...
def aba_coleta(df: pd.DataFrame) -> None:
    ...
    if salvar:
        ...
        df_manual = pd.DataFrame([{
            "Nome da obra": nome, "Autor": autor, "Preço": preco, "Técnica": tecnica,
...
def aba_comparativo(df: pd.DataFrame) -> None:
    ...
    with st.expander("Metodologia de similaridade e níveis de valor justo", expanded=False):
        st.markdown(
            "O comparativo segue uma lógica de **abordagem de mercado**. A pontuação total é de 100 pontos, distribuída por técnica, dimensões, preço, ano e autor. **Nível 1:** similaridade ≥ 80. **Nível 2:** entre 50 e 79,99. **Nível 3:** abaixo de 50."
        )
    ...
    for _, row in res.iterrows():
        ...
        with b:
            ...
            m1.metric("Preço", row["Preço"])
            m2.metric("Técnica", limpar_texto(row["Técnica"]))
            m3.metric("Dimensões", limpar_texto(row["Dimensões"]))
            m4.metric("Ano", limpar_texto(row["Ano"]))
            p1, p2, p3, p4, p5 = st.columns(5)
            p1.metric("Pts Técnica", row["Pts Técnica"])
            p2.metric("Pts Dimensões", row["Pts Dimensões"])
            p3.metric("Pts Preço", row["Pts Preço"])
            p4.metric("Pts Ano", row["Pts Ano"])
            p5.metric("Pts Autor", row["Pts Autor"])
