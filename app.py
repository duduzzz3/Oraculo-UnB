...
def normalizar_tecnica(valor: Any) -> str:
    ...
    if any(x in t for x in ["mixed media", "tecnica mista", "mista", "mixed"]):
    ...
    if any(x in t for x in ["lithograph", "lithography", "litografia"]):
        return "Litografia"
    if any(x in t for x in ["engraving", "etching", "woodcut", "linocut", "monotype", "monoprint", "printmaking", "gravura", "mixografia"]):
        return "Gravura"
    ...
    if any(x in t for x in ["digital", "new media", "arte digital"]):
        return "Arte digital"
    if any(x in t for x in ["ceramic", "ceramics", "ceramica", "porcelain"]):
        return "Cerâmica"
    if any(x in t for x in ["resin", "resina"]):
        return "Resina"
    if any(x in t for x in ["metal", "bronze", "steel", "iron", "aluminum", "aluminium", "copper", "brass"]):
        return "Metal"
    if any(x in t for x in ["wood", "madeira"]):
        return "Madeira"
    if any(x in t for x in ["textile", "fabric", "fiber", "fibre", "tapestry", "tecido", "textil"]):
        return "Têxtil"
    if any(x in t for x in ["sculpture", "escultura"]):
        return "Escultura"
    if any(x in t for x in ["painting", "pintura", "drawing", "desenho", "print"]):
        return "Outros"
    return "Outros"
...
def obter_cotacao_dolar() -> float:
    """Busca USD-BRL com múltiplas fontes.

    Em deploy online algumas APIs podem falhar por bloqueio temporário, SSL ou
    limite de conexão. Por isso tentamos fontes diferentes antes do fallback.
    """
    headers = {"User-Agent": "Mozilla/5.0"}

    # 1) AwesomeAPI, fonte brasileira usual.
    for url in [
        ...
        try:
            ...
            cot = parse_preco(bid)
            if cot and 2 <= cot <= 15:
                return round(float(cot), 4)
        except Exception:
            pass

    # 2) Frankfurter/ECB: retorna rates.BRL.
    try:
        resp = requests.get("https://api.frankfurter.app/latest?from=USD&to=BRL", timeout=10, headers=headers)
        resp.raise_for_status()
        cot = parse_preco(resp.json().get("rates", {}).get("BRL"))
        if cot and 2 <= cot <= 15:
            return round(float(cot), 4)
    except Exception:
        pass

    # 3) open.er-api.com: retorna rates.BRL.
    try:
        resp = requests.get("https://open.er-api.com/v6/latest/USD", timeout=10, headers=headers)
        resp.raise_for_status()
        cot = parse_preco(resp.json().get("rates", {}).get("BRL"))
        if cot and 2 <= cot <= 15:
            return round(float(cot), 4)
    ...
    # Último fallback: evita quebrar o app, mas deixa claro que é estimativo.
    return 5.0
...
def aba_inicio(df: pd.DataFrame) -> None:
    ...
    c2.metric("Preço médio", dinheiro(df["preco"].dropna().mean()) if not df.empty and "preco" in df else SEM_INFO)
    c3.metric("Sites", numero(df["site"].nunique()) if not df.empty else "0")
    ...
    if not df.empty:
        ...
        st.dataframe(df.groupby("site").agg(obras=("id", "count"), preco_medio=("preco", "mean")).reset_index(), use_container_width=True)
...
def aba_coleta(df: pd.DataFrame) -> None:
    ...
    if salvar:
        ...
        df_manual = pd.DataFrame([{"Nome da obra": nome, "Autor": autor, "Preço": preco, "Técnica": tecnica, "Técnica original": tecnica, "Dimensões": dimensoes, "Ano da obra": ano, "Descrição": descricao, "Link da obra": link_final, "Link da imagem da obra": imagem_final}])
        inseridas = inserir_obras(dataframe_para_banco(df_manual, "Manual", "manual", obter_cotacao_dolar()))
        st.success(f"{inseridas} obra salva.")
        st.rerun()

    st.divider()
    st.subheader("Limpeza e correção")
    col1, col2, col3 = st.columns(3)
    site_del = col1.selectbox("Apagar por site", ["Todos"] + list(SITES.keys()) + ["Manual"])
    if col2.button("Apagar dados selecionados"):
        total = excluir_por_site(None if site_del == "Todos" else site_del)
        st.success(f"{total} registro(s) apagado(s).")
        st.rerun()
    if col3.button("Corrigir/sanear acervo já salvo"):
        total = sanear_acervo()
        st.success(f"{total} registro(s) corrigido(s).")
        st.rerun()


def imagem_para_exibir(valor: str) -> str | None:
    texto = limpar_texto(valor)
    if texto == SEM_INFO:
        return None
    if texto.startswith("http") or Path(texto).exists():
        return texto
    return None
...
def aba_comparativo(df: pd.DataFrame) -> None:
    ...
    with st.expander("Metodologia de similaridade e níveis de valor justo", expanded=False):
        st.markdown(
            """
            O comparativo segue uma lógica de **abordagem de mercado**: primeiro procura evidências observáveis em obras idênticas ou comparáveis; depois mede o quanto essas evidências precisam de ajuste. A pontuação total é de 100 pontos:

            | Critério | Peso | Justificativa |
            |---|---:|---|
            | Técnica utilizada | 28 | Técnica, materialidade, suporte, durabilidade e mercado. |
            | Dimensões | 25 | Obras de escala semelhante exigem menos ajuste. |
            | Preço observado | 22 | Evidência monetária de mercado, ponderada por diferença de atributos. |
            | Ano da obra | 15 | Proximidade temporal reduz diferenças de fase artística e mercado. |
            | Autor | 10 | Mesmo autor reforça comparabilidade, mas não substitui os demais atributos. |

            **Nível 1:** similaridade ≥ 80.  
            **Nível 2:** similaridade entre 50 e 79,99.  
            **Nível 3:** similaridade abaixo de 50.
            """
        )
    ...
    col_img, col_info = st.columns([1.05, 2.2], vertical_alignment="top")
    with col_img:
        img = imagem_para_exibir(ref.get("link_imagem"))
        if img:
            st.image(img, use_container_width=True)
        else:
            st.info("Imagem não disponível para esta obra.")
    with col_info:
        st.markdown(f"### {limpar_texto(ref.get('nome_obra'))}")
        st.caption(f"{limpar_texto(ref.get('autor'))} · {limpar_texto(ref.get('site'))}")
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Preço", dinheiro(ref.get("preco")))
        m2.metric("Técnica", limpar_texto(ref.get("tecnica")))
        m3.metric("Dimensões", limpar_texto(ref.get("dimensoes")))
        m4.metric("Ano", limpar_texto(ref.get("ano_obra")))
        descricao_ref = limpar_texto(ref.get("descricao"))
        if descricao_ref != SEM_INFO:
            st.markdown(f"**Descrição:** {descricao_ref}")
        link_ref = limpar_texto(ref.get("link_obra"))
        if link_ref != SEM_INFO and link_ref.startswith("http"):
            st.link_button("Abrir obra original", link_ref)

    resultados = montar_resultados_comparativo(df, ref)
    if resultados.empty:
        st.info("Não há comparáveis para esta obra.")
        return

    st.divider()
    st.subheader("Filtros de similaridade")
    f1, f2, f3, f4 = st.columns(4)
    min_sim = f1.slider("Similaridade mínima", 0, 100, 0, 5)
    niveis = f2.multiselect("Níveis", ["Nível 1", "Nível 2", "Nível 3"], default=["Nível 1", "Nível 2", "Nível 3"])
    sites = f3.multiselect("Sites comparáveis", sorted(resultados["Site"].dropna().unique().tolist()), default=[])
    top_n = f4.number_input("Quantidade máxima exibida", min_value=1, max_value=500, value=min(30, len(resultados)), step=5)

    c1, c2, c3, c4 = st.columns(4)
    apenas_mesma_tecnica = c1.checkbox("Somente mesma técnica")
    apenas_mesmo_autor = c2.checkbox("Somente mesmo autor")
    ocultar_sem_preco = c3.checkbox("Ocultar sem preço")
    ocultar_sem_imagem = c4.checkbox("Ocultar sem imagem")

    filtrado = resultados.copy()
    filtrado = filtrado[filtrado["Similaridade (%)"] >= min_sim]
    if niveis:
        filtrado = filtrado[filtrado["Nível"].isin(niveis)]
    if sites:
        filtrado = filtrado[filtrado["Site"].isin(sites)]
    if apenas_mesma_tecnica:
        filtrado = filtrado[filtrado["Técnica igual"]]
    if apenas_mesmo_autor:
        filtrado = filtrado[filtrado["Autor igual"]]
    if ocultar_sem_preco:
        filtrado = filtrado[pd.notna(filtrado["Preço numérico"])]
    if ocultar_sem_imagem:
        filtrado = filtrado[filtrado["Imagem"].apply(lambda x: imagem_para_exibir(x) is not None)]
    filtrado = filtrado.head(int(top_n))

    st.caption(f"{len(filtrado)} comparável(is) exibido(s), em ordem decrescente de similaridade.")
    if filtrado.empty:
        st.warning("Nenhuma obra atende aos filtros selecionados.")
        return

    st.subheader("Tabela de similaridade")
    tabela = filtrado[[
        "Nível", "Similaridade (%)", "Nome da obra", "Autor", "Site", "Preço", "Técnica", "Dimensões", "Ano",
        "Técnica igual", "Autor igual", "Proximidade área", "Diferença preço", "Diferença ano",
        "Pts Técnica", "Pts Dimensões", "Pts Preço", "Pts Ano", "Pts Autor",
    ]].copy()
    st.dataframe(
        tabela,
        ...
        column_config={
            "Similaridade (%)": st.column_config.ProgressColumn(
                "Similaridade",
                help="Pontuação total de 0 a 100 segundo os critérios ponderados.",
                min_value=0,
                max_value=100,
                format="%.2f%%",
            ),
            "Técnica igual": st.column_config.CheckboxColumn("Técnica igual"),
            "Autor igual": st.column_config.CheckboxColumn("Autor igual"),
        },
    )
    ...
    for _, row in filtrado.iterrows():
        with st.container(border=True):
            c_img, c_info = st.columns([0.95, 2.6], vertical_alignment="top")
            with c_img:
                img = imagem_para_exibir(row.get("Imagem"))
                if img:
                    st.image(img, use_container_width=True)
                else:
                    st.caption("Sem imagem")
            with c_info:
                topo1, topo2 = st.columns([2.4, 1])
                with topo1:
                    st.markdown(f"### {limpar_texto(row.get('Nome da obra'))}")
                    st.caption(f"{limpar_texto(row.get('Autor'))} · {limpar_texto(row.get('Site'))} · {limpar_texto(row.get('Nível'))}")
                with topo2:
                    st.metric("Similaridade", percentual(row.get("Similaridade (%)")))
                st.progress(min(max(float(row.get("Similaridade (%)", 0)) / 100, 0), 1))
                d1, d2, d3, d4 = st.columns(4)
                d1.write(f"**Preço:** {limpar_texto(row.get('Preço'))}")
                d2.write(f"**Técnica:** {limpar_texto(row.get('Técnica'))}")
                d3.write(f"**Dimensões:** {limpar_texto(row.get('Dimensões'))}")
                d4.write(f"**Ano:** {limpar_texto(row.get('Ano'))}")
                e1, e2, e3, e4, e5 = st.columns(5)
                e1.metric("Pts técnica", f"{row.get('Pts Técnica', 0):.2f}")
                e2.metric("Pts dimensões", f"{row.get('Pts Dimensões', 0):.2f}")
                e3.metric("Pts preço", f"{row.get('Pts Preço', 0):.2f}")
                e4.metric("Pts ano", f"{row.get('Pts Ano', 0):.2f}")
                e5.metric("Pts autor", f"{row.get('Pts Autor', 0):.2f}")
                st.caption(
                    f"Critérios: técnica igual = {'sim' if row.get('Técnica igual') else 'não'} · "
                    f"autor igual = {'sim' if row.get('Autor igual') else 'não'} · "
                    f"proximidade de área = {row.get('Proximidade área')} · "
                    f"diferença de preço = {row.get('Diferença preço')} · "
                    f"diferença de ano = {row.get('Diferença ano')}"
                )
                link = limpar_texto(row.get("Link"))
                if link != SEM_INFO and link.startswith("http"):
                    st.link_button("Abrir obra comparável", link)
...
def main() -> None:
    ...
    inicializar_banco()
    df = carregar_acervo()
    tabs = st.tabs(["Início", "Coleta", "Acervo", "Comparativo", "Informações", "Quem somos nós"])
    with tabs[0]:
        aba_inicio(df)
    with tabs[1]:
        aba_coleta(df)
